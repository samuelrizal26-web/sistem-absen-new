  ...
  Future<void> _login() async {
    if (_employeeIdController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('ID Karyawan tidak boleh kosong'), backgroundColor: Colors.orange,),
      );
      return;
    }
    ...
    try {
      final url = Uri.parse('https://sistem-absen-production.up.railway.app/api/employees/${_employeeIdController.text}');
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Login Gagal: Karyawan tidak ditemukan'), backgroundColor: Colors.red,),
        );
      }
    } catch (e) {
      if (!mounted) return;
